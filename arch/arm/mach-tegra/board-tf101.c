...
static noinline void __init tf101_bt_rfkill(void)
{
	/*Add Clock Resource*/
	clk_add_alias("bcm4329_32k_clk", tf101_bcm4329_rfkill_device.name, \
				"blink", NULL);

	platform_device_register(&tf101_bcm4329_rfkill_device);
...
static void tf101_power_off(void)
{
	int ret;

	ret = tps6586x_power_off();
	if (ret)
		pr_err("tf101: failed to power off\n");

	while(1);
}

static void __init tf101_power_off_init(void)
{
	pm_power_off = tf101_power_off;
}
